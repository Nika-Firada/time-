...
class DashboardItem {
    ...
    createMarkup(){
        ...
        <div class="dashborard_item dashborard_item--${id}">
            <article class="tracking-card">
                <div class="tracking-card_header">
                    <h4 class="tracking-card_title">${title}</h4>
                    <img class="tracking-card_menu" src="images/icon-ellipsis.svg" alt="#">
                </div>
                <div class="tracking-card_body">
                    <div class="tracking-card_time">${current}hrs</div>
                    <div class="tracking-card_prev-period">Last${DashboardItem.PERIODS[this.view]}-${previous}hrs</div>
                </div>
            </article>
        </div>
        `);
        this.time = this.container.querySelector(`.dashboard_item--${id} .tracking-card_time`);
        this.time = this.container.querySelector(`.dashboard_item--${id} .tracking-card_prev-period`);
    }
    changeView(view) {
